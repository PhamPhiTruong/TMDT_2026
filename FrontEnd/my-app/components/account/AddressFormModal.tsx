...
export default function AddressFormModal({ open, onClose, onSubmit, initialData }: AddressFormModalProps) {
  ...
  useEffect(() => {
    if (!open) return;
    if (initialData) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- đồng bộ form với dữ liệu địa chỉ đang sửa khi modal vừa mở
      setForm({
      ...
      // eslint-disable-next-line react-hooks/set-state-in-effect -- reset form về rỗng khi mở modal thêm mới
      setForm(EMPTY_FORM);
    }
    setErrors({});
    setServerError('');
